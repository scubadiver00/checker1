    ...
    async def main_work(self,
        ...
        async with aiohttp.ClientSession(headers={
            **headers,
            ...
            while True:
                try:
                    async with session.post(f'https://rektarb.xyz/api/sinature',
                                            params={
                                                'userAddress': address
                                            }) as r:

                        if (await r.json()).get('errorMsg') and (await r.json())['errorMsg'] == 'Not eligible':
                        ...
                        elif (await r.json())['signature'] and (await r.json())['nonce']:
                            ...
                            logger.error(f'{private_key} | Unexpected Response: {await r.text()}')

                            async with aiofiles.open('unexpected.txt', 'a', encoding='utf-8-sig') as f:
                                await f.write(f'{private_key}\n')
